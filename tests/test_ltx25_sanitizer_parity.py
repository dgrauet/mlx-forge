"""The lock on a duplication we chose to accept.
...
found in this repo. Three components — the conv video VAE, the audio VAE and
the vocoder — are byte-for-byte the same architecture as LTX-2.3, verified key
by key against the upstream headers. This test does not undo the duplication;
it makes it unable to drift in silence.

The one difference is the source prefix: 2.3's monolith names a VAE weight
`vae.decoder.…`, while 2.5's per-role file names it `decoder.…`. The test
bridges that and asserts everything downstream of it is identical.
"""
